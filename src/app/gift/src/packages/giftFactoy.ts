  ...
  async createGifts(params: CreateGiftsParams): Promise<string>{
      ...
      try {
        const inputConfig = {
          ...
          totalReward: BigInt(totalReward * 10**rewardToken.decimal),
        ...
        const response = await tokenContract.approve(this.contractAddress,String(totalReward * 10**rewardToken.decimal))
         await response.wait()
